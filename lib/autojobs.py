...
def discover_jobs(config, jobs, state):
    ...
    for release_stub in releases_result["releases"]:
        ...
        try:
            candidate_job = build_job_from_release(release_payload, new_episode_numbers, automation)
        ...
        existing_job = find_matching_job(updated_jobs, candidate_job)
        if existing_job is None:
            updated_jobs.append(candidate_job)
            created_jobs += 1
        else:
            merged_range = merge_episode_ranges(existing_job["episodes_range"], new_episode_numbers)
            if merged_range != existing_job["episodes_range"]:
                existing_job["episodes_range"] = merged_range
...
def build_job_from_release(release_payload, new_episode_numbers, automation):
    title = extract_release_title(release_payload)
