...
- `/addseasons Название ; 1-5 ; 0` — объединить несколько сезонов; по одной magnet-ссылке на сезон, без диапазона серий
...
magnet:?xt=urn:btih:МАГНЕТ_СЕЗОНА_5
```

Серии каждого сезона определяются из torrent metadata. Диапазон обязан начинаться с 1 и не иметь внутренних пропусков. Конечное число серий автоматически подтвердить нельзя: torrent с `1-12` может быть полным сезоном или неполной раздачей.
