...
- `003`
- `001,003,005-007`
- `500-600?` — взять доступные серии из torrent metadata, разрешив пропуски

Без `?` диапазон остаётся строгим: отсутствие любой запрошенной серии останавливает job.
С `?` отсутствующие номера записываются в manifest и Telegram «Подробно» как warning.
...
`support_banner` управляет периодическим баннером поддержки:
- `enabled` — глобальное включение;
- `path` — путь к прозрачному PNG;
- `interval_episodes` — период для compilation; по умолчанию каждая 6-я серия;
- `duration_seconds` — полный цикл показа;
- `slide_seconds` — длительность въезда и выезда;
- `width_px` — ширина баннера после масштабирования;
- `bottom_margin_px` — отступ от нижнего края.

В публичной одиночной серии баннер показывается один раз точно в середине серии. В compilation
он показывается в каждой `interval_episodes`-й серии по порядку выбранных серий. Multi-season
использует сквозной счётчик без сброса между сезонами и частями. Время рассчитывается после
удаления OP/ED. Баннер въезжает снизу по центру и уезжает обратно. Для приватных/Donut-релизов
с `vk_privacy_view = 5` баннер всегда отключён; 4K-worker его не использует.
...
- `/priority <номер>` — поднять ожидающий job над остальной очередью; последний поднятый выполняется первым
...
- `/add Название ; 001-003 ; magnet:?xt=... ; 2 ; 0 ; HEVC` — обычный render; фильтр `HEVC` необязателен; `?` после диапазона разрешает отсутствующие серии
- `/addmulti Название ; 001-148 ; 1 ; 0` — compilation из нескольких magnet-источников; каждая magnet-ссылка указывается с новой строки, необязательный фильтр добавляется после `;`; `?` после диапазона разрешает отсутствующие серии
- `/addseasons Название ; 1-5 ; 0` — объединить несколько сезонов; magnet-ссылки идут по порядку, лишние ссылки считаются частями последнего сезона; `?` после диапазона сезонов разрешает внутренние пропуски серий
- `/add4k Название ; 25 ; magnet:?xt=... ; 2 ; HEVC` — серии `001-025` в 4K для Donut; фильтр необязателен; `?` после количества или диапазона разрешает отсутствующие серии

`/priority` не прерывает активный render. Поднятый job становится следующим в очереди
своего worker (основной или 4K), а boost
автоматически снимается при старте. Несколько поднятых job выполняются в обратном порядке
команд: после `/priority 1`, затем `/priority 2` порядок будет `2`, `1`, остальные.
